...
public class ShowCheckUI extends MainPanel {
	...
	private void initData() {
		for (int i = 0; i < enterList.size(); i++) {
			String[] data = { "入库单", enterList.get(i).getId().substring(12, enterList.get(i).getId().length()),
					enterList.get(i).getDate(), enterList.get(i).getList().size() + "" };
			table.getTableModel().addRow(data);
		}
		for (int i = 0; i < exitList.size(); i++) {
			String[] data = { "出库单", exitList.get(i).getId().substring(11, exitList.get(i).getId().length()),
					exitList.get(i).getDate(), exitList.get(i).getList().size() + "" };
			table.getTableModel().addRow(data);
		}
	}
